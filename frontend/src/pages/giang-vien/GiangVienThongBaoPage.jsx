...
const GiangVienThongBaoPage = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/giang-vien/thong-bao');
      setData(res.data || []);
  ...
  const handleDaDoc = async (id) => {
    try {
      await api.put(`/giang-vien/thong-bao/${id}/doc`);
      window.dispatchEvent(new CustomEvent('giang-vien-thong-bao-updated'));
      fetchData();
...
export default GiangVienThongBaoPage;
