...
import { Card, Table, Button, Modal, Form, Select, message, Popconfirm, Tag } from 'antd';
import { PlusOutlined, ReloadOutlined, DeleteOutlined } from '@ant-design/icons';
import api from '../../services/api';

const TRANG_THAI_COLOR = { hoat_dong: 'green', huy: 'red' };

const AdminSVLHPPage = () => {
  ...
  const fetchSinhVien = async () => {
    try {
      const res = await api.get('/admin/nguoi-dung', {
        params: { vai_tro: 'sinh_vien', trang_thai: 'hoat_dong', limit: 100 },
      });
      const rows = res.data?.data || [];
      setSinhVienList(
        rows
          .filter((sv) => Number.isInteger(sv.sinh_vien_id) && sv.sinh_vien_id > 0)
          .map((sv) => ({
            value: sv.sinh_vien_id,
  ...
  const fetchData = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.get('/admin/sinh-vien-lhp', { params: { lop_hoc_phan_id: selectedLHP } });
      setData(res.data || []);
  ...
  const handleSubmit = async (values) => {
    try {
      await api.post('/admin/sinh-vien-lhp', {
  ...
  const handleDelete = async (record) => {
    try {
      await api.delete('/admin/sinh-vien-lhp', {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Popconfirm title="Xóa sinh viên khỏi lớp học phần?" onConfirm={() => handleDelete(record)} okText="Xóa" cancelText="Hủy">
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card>
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
          <Select
            value={selectedHK}
            onChange={val => { setSelectedHK(val); setSelectedLHP(null); }}
            style={{ width: 200 }}
      ...
      >
        <Form form={form} layout="vertical" onFinish={handleSubmit}>
          ...
          <Form.Item style={{ marginBottom: 0, textAlign: 'right' }}>
            <Button onClick={() => { setShowModal(false); form.resetFields(); }} style={{ marginRight: 8 }}>Hủy</Button>
            <Button type="primary" htmlType="submit" style={{ backgroundColor: '#C63633', borderColor: '#C63633' }}>Xếp SV</Button>
          </Form.Item>
        </Form>
      </Modal>
    </div>
